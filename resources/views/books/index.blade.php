    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-6">
                ...
                <x-primary-button tag="a" href="{{ route('books.print')}}" target="blank">Print Data Buku</x-primary-button>
                <x-primary-button tag="a" href="{{ route('books.export')}}" target="blank">Export Data Buku</x-primary-button>
                <x-primary-button 
                        x-data=""
                        x-on:click.prevent="$dispatch('open-modal', 'import-book')"
                >{{ __('Import Data Buku') }}</x-primary-button>
            </div>
                ...
                @foreach($books as $book)
                    <tr>
                        ...
                        <td>
                            ...
                            <form action="{{ route('books.destroy', $book->id) }}" method="post" onsubmit="return confirm('APAKAH YAKIN?');">
                                @csrf
                                @method('delete')
